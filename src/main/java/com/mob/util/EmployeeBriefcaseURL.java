...
public class EmployeeBriefcaseURL  extends TableDecorator {
	
    /**
     * Logger for logging messages.
     */
    private static final MOBLogger LOGGER =
        new MOBLogger(EmployeeBriefcaseURL.class);
    ...
    public Object getItemURL() {
    	...
    	System.out.println("In Get Item URL:::----");
        Item item = (Item) getCurrentRowObject();
        StringBuffer retString = new StringBuffer();


        retString.append("<div  id='name" + item.getId() + "' style='display:block;'>");
        retString.append("<a target='_blank' class='briefCaseLink' href="+item.getItemURL()+ ">" +item.getItemURL()+"</a>");
        retString.append("</div>");
    ...
    public Object getUrl1() {
    	LOGGER.info("Inside getUrl");
        Item item = (Item) getCurrentRowObject();
        StringBuffer retString = new StringBuffer();

        retString.append("<div id='url" + item.getId() + "' style='display:block'>");
        retString.append(item.getItemURL());
        retString.append("</div>");
        retString.append("<div id='url1" + item.getId() + "' style='display:none'>");
        retString.append("</div>");
        retString.append("</div>");
    ...
    public Object getLinks() {
       ...
       // retString.append("<a href=''><img src='../styles/default/images/mobc_prof_action_btn-envelope.gif'/></a>");
        ...
        retString.append("<input type='image'   src='../styles/default/images/grn_save_btn-up.png' alt='Save' onclick=\"editBriefcaseDetailsURL('" + item.getId() + "')\"/>");
        retString.append("<br>");
