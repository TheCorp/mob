...
public class GetEmployeeContactsForSkypeAction extends ActionSupport {
	/**
	 * Logger for logging messages.
	 */
	private static final MOBLogger LOGGER = new MOBLogger(
			GetEmployeeContactsForSkypeAction.class);
	...
	public String getContacts() {
		...
		contactsForSkype = contactEmployeeService.findEmployeeContactsForSkype(
				user, name);
		try {
			...
			while (it.hasNext()) {
				...
				if (contact.getContactOwner().getPicture() != null) {
					image = (String) contact.getContactOwner()
				...
				if (contact.getContactOwner().getResizedMinPicture() != null) {
					...
					contact.getContactOwner().setResizedMinPicture(
							resizedMinPicture);
		...
		LOGGER.info("Size " + contactsForSkype.size());
		return "success";
	}
