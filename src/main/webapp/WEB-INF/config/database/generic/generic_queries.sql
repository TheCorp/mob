...
UserModel.selectProfileByName = SELECT id,name,user_id FROM profile WHERE LOWER(name) LIKE LOWER(?)
UserModel.selectUserProfileById = SELECT * FROM profile WHERE user_id = ?
...
PostModel.selectById = SELECT p.post_id, topic_id, forum_id, p.user_id,p.profile_id, post_time, poster_ip, enable_bbcode, enable_html, \
...
PostModel.addNewPost = INSERT INTO jforum_posts (topic_id, forum_id, user_id, post_time, poster_ip, enable_bbcode, enable_html, enable_smilies, enable_sig, post_edit_time, need_moderate,profile_id) \
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?,?)
...
PostModel.selectAllByTopicByLimit = SELECT p.post_id, topic_id, forum_id, p.user_id, post_time, poster_ip, enable_bbcode, p.attach, \
	enable_html, enable_smilies, enable_sig, post_edit_time, post_edit_count, status, pt.post_subject, pt.post_text, username, p.need_moderate,p.profile_id \
	FROM jforum_posts p, jforum_posts_text pt, jforum_users u \
	WHERE p.post_id = pt.post_id \
	AND topic_id = ? \
	AND p.user_id = u.user_id \
...
PostModel.selectByUserByLimit = SELECT p.post_id, topic_id, forum_id, p.user_id, post_time, poster_ip, enable_bbcode, p.attach,p.profile_id, \
...
PostModel.findUserProfiles=SELECT name FROM profile WHERE id = ? 
...
ForumModel.lastPostInfo = SELECT post_time, p.topic_id,p.profile_id,t.topic_replies,t.profile_id, post_id, u.user_id, username \
...
TopicModel.selectById = SELECT t.*, p.user_id AS last_user_id,p.profile_id AS last_profile_id, p.post_time, p.attach AS attach \
	FROM jforum_topics t, jforum_posts p \
	WHERE t.topic_id = ? \
	AND p.post_id = t.topic_last_post_id
	
TopicModel.selectRaw = SELECT topic_id, forum_id, topic_title, user_id,profile_id,topic_views, topic_replies, topic_status, topic_vote_id, topic_type, \
	topic_first_post_id, topic_last_post_id, moderated, topic_time, topic_moved_id \
	FROM jforum_topics WHERE topic_id = ?

TopicModel.selectAllByForumByLimit = SELECT t.*, p.user_id AS last_user_id,p.profile_id AS last_profile_id, p.post_time, p.attach AS attach \
	FROM jforum_topics t, jforum_posts p \
...
TopicModel.addNew = INSERT INTO jforum_topics (forum_id, topic_title, user_id, topic_time, topic_first_post_id, topic_last_post_id, topic_type, moderated,profile_id) \
	VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)
...
TopicModel.selectRecentTopicsByLimit = SELECT t.*, p.user_id AS last_user_id,p.profile_id AS last_profile_id, p.post_time, p.attach AS attach \
	FROM jforum_topics t, jforum_posts p \
	WHERE p.post_id = t.topic_last_post_id \
	AND p.need_moderate = 0 \
...
TopicModel.selectForNewMessages = SELECT t.*, p.user_id AS last_user_id,p.profile_id AS last_profile_id, p.post_time, p.attach AS attach \
	FROM jforum_topics t, jforum_posts p \
...
TopicModel.selectHottestTopicsByLimit = SELECT t.*, p.user_id AS last_user_id,p.profile_id AS last_profile_id, p.post_time, p.attach AS attach \
    FROM jforum_topics t, jforum_posts p \
    WHERE p.post_id = t.topic_last_post_id \
    AND p.need_moderate = 0 \
...
TopicModel.selectByUserByLimit = SELECT t.*, p.user_id AS last_user_id,p.profile_id AS last_profile_id, p.post_time, p.attach AS attach \
	FROM jforum_topics t, jforum_posts p \
	WHERE p.post_id = t.topic_last_post_id \
	AND t.user_id = ? \
	AND p.need_moderate = 0 \
...
PrivateMessageModel.add = INSERT INTO jforum_privmsgs ( privmsgs_type, privmsgs_subject, privmsgs_from_userid, \
	...
	privmsgs_attach_sig,privmsgs_from_profileid) \
	VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?,? )
...
PrivateMessageModel.baseListing = SELECT pm.privmsgs_type, pm.privmsgs_id, pm.privmsgs_date, pm.privmsgs_subject,pm.privmsgs_from_profileid,u.user_id, u.username \
...
PrivateMessageModel.selectById = SELECT p.*,p.privmsgs_from_profileid,pt.privmsgs_text \
