	$(document).ready(function() {
		// Call displayChoice from signupPage.jsp include script which has query param of user selection
		displayChoice();
	});
	...
	*/
	...
	function checkForm()
	{
		 $("#acceptTermsOfServiceWarnMsgEmployee").html("");	
	    ...
	    if(joinfreeValidate == 1)
	     ...
	     }else if(joinfreeValidate == 8)
	     {
	      ...
	      {
	         joinfreeValidate = 0;
	         return validateTermsAndConditions('acceptTermsOfServiceWarnMsgEmployee',$('#acceptEmployeeCondi').is(':checked'));
	      
	     } 
	
	}
	
	/*Added for checking the Temrs and conditiond of MOB accepted or not
	 *If accepted return true else return false 
	 */	
	function validateTermsAndConditions(spanLoc,isAccept){
	
		if(isAccept){
			return true;
		}
		else{
			$("#"+spanLoc).html("Please accept the terms before you join.");
			return false;
		}
	}
	...
	function checkDateForSignup(txtID,imageArea,messageArea)
	{
        ...
        if(input!="" && isDateForSignup(input,imageArea,messageArea)==false){
       		  if(joinfreeValidate == 0)
          	   joinfreeValidate = 3 
			   return false
        }
       		...
       		var currentMnt=today.getMonth()+2; 
       		var currentYear=today.getFullYear()+1;

			if (year>=currentYear){
	            flag="false"; 
	            msg="Plese select valid date.";
	            setMessageForSignup(msg,flag,imageArea,messageArea); 
	            return flag;
            }else if (year<=currentYear && month>=currentMnt){
	            flag="false";
	            msg="Plese select valid date.";
	            setMessageForSignup(msg,flag,imageArea,messageArea); 
	            return flag;
	...
	function validateEmpCity(txtID,imageArea,messageArea){
	 ...
	 var minLength = 25; 
	             ...
	             /* if(empCity==""){
	                    ...
	                    // check for minimum length
	                    if (empCity!="" && empCity.length > minLength)
	                     {
	...
	/*function validateEmpState(txtID,imageArea,messageArea){
	 var flag  ="true";      
	 var message="" 
	 var empState = document.getElementById(txtID).value;
	     // var empState = $("#empState").val();
	    
	       var minLength = 25; 
	             
	                    // check for minimum length
	                    if (empState!="1"&&empState.length > minLength){
	                         ...
	                         setMessageForSignup(message,flag,imageArea,messageArea);
	                         return false;
	                     }else if(empState!="1"&&trimFieldVal(empState)==""){
	...
	*/
	function validateEmpZipCode(txtID,imageArea,messageArea)
	{
	 var flag  ="true";    
	 var message=""  
	 var empZipCode = document.getElementById(txtID).value;
	 var regExpZipCode = new RegExp(/(^\d{5}$)|(^\d{5}-\d{4}$)/);
	      ...
	      /*  if(empZipCode==""){
	         ...
	         if(empZipCode!="" && trimFieldVal(empZipCode)==""){
		         message="Please enter Postal code"
		         flag="false"
		         if(joinfreeValidate == 0)
		         joinfreeValidate = 7
		         setMessageForSignup(message,flag,imageArea,messageArea);
		         return false;
	        }else if (empZipCode!="" && !regExpZipCode.test(empZipCode)){
...
function checkEmprVerificationCode1(){
	...
	function validateEmployer(txtID,imageArea,messageArea) 
	{
	      ...
	      if (txtElement.value=="")
	         ...
	         else   if (txtElement.value.length>20) 
		      {
	...
	/*function checktermsandcondtions(chk,imageArea,messageArea){
	  if (chk.checked == 1)
	    alert("Thank You");
	     else
	...
	function  validateEmprAddress(txtID,imageArea,messageArea){
	    ...
	    var minLength = 45; 
	    var flag="true";
	    var emprAddress = document.getElementById(txtID).value;
	             ...
	             /* if(emprAddress==""){
	              ...
	              if (emprAddress!="" && emprAddress.length > minLength){
		                ...
		                setMessageForEmprAccountValidation(message,flag,imageArea,messageArea);
		                return false;
	              }else if(emprAddress!="" && trimFieldVal(emprAddress)==""){
		               message="Please enter Address"
		               flag="false"
		               if(joinfreeValidate == 0)
		               joinfreeValidate = 7
	...
	function validateEmprCity(txtID,imageArea,messageArea){
		...
		var emprCity = document.getElementById(txtID).value;
	              ...
	              if (emprCity!="" && emprCity.length > minLength){
                         message="City should be within" + minLength + " characters long. Try again."
                         flag="false"
                         if(joinfreeValidate == 0)
                         joinfreeValidate = 8
                     ...
                     }else if(emprCity!="" && trimFieldVal(emprCity)==""){
	                      message="Please enter City"
	                      flag="false"
	                      if(joinfreeValidate == 0)
	                      joinfreeValidate = 8
	...
	var minLength = 40; 
	var flag="true";
	var emprState = document.getElementById("nameURL").value;
	...
	function addemplrwebURL(nameURL,webimageArea,webMessageArea) 
{
...
//	alert("HII");
	if(nameOfURL==""){
			...
			msg="Please enter name of URL."          
	        flag="false"
	   setMessageForEmprAccountValidation(msg,webimageArea,webMessageArea);
	...
	*/
	...
	function sjcap(type,jfldcls){
		...
		["60ee0bc62638fccf2d37ac27a634a9e9", "68e2d83709f317938b51e53f7552ed04", "f4c9385f1902f7334b00b9b4ecd164de",
		 ...
		 "015f28b9df1bdd36427dd976fb73b29d", "755f85c2723bb39381c7379a604160d8"];
		  ...
		  var text = "<p><input type=\"text\" tabindex='16' id=\"" + jfldid + "\" name=\"" + jfldid + "\" class=\"" + jfldcls +
		   "\" size=\"" +  jfldsz + "\"><\/p><p style='padding-left: 118px;'><img src=\"" + imgdir + imgid + ".jpg\" width=\"200\" height=\"80\" alt=\"\"><\/p>";
	...
	function jcap(){
			...
			var message="Enter the code as it is shown.";
			var fontColor="red";
			$("#imageAreaVcode").html("");
			$("#messageAreaVcode").html("<font color=" + fontColor + ">"+ message + " </font>");
	...
	function validate1(txtID,imageArea,messageArea) 
	{
...
function checkEmployerForm()
	...
	*/
	function checkEmprForm2(){
		 ...
		 $("#acceptTermsOfServiceWarnMsgEmployer").html("");
		...
		//Validating Organization name
		 ...
		 //Validating Re-Enter Email 
		 reEnterEmprEmail('reEnterEmployeremail','imageArea111','msgArea111');
		 
		 //Validating Password & Re-Enter Password
		 validateEmprPwd('imageArea22','msgArea22');
		 confirmEmprPwd('confirmimageArea22','confirmmsgArea22');
		 ...
		 validateEmpZipCode('emplrZipCode','emplrZipimageArea','emplrZipmessageArea');
		  
		 validatePhoneNumbernew('emplrphoneNumberId','pnImageArea5','pnMessageArea5');
		 validateMissionStateMent('missionstatementId','missionStmtimageArea','missionStmtmsgArea');
		 validateEmprUpLoadImage('uploadpic','imageAreaImage','messageAreaImage');
		 
		 
	
	 // validate1('','imgArea','msgArea');
	 
	//  checktermsandcondtions(chk,'imageArea55','messageArea55');
	// validateOrgInfo('orgInfo','imageArea33','msgArea33');
	// validateCultureInfo('culture','imageArea44','msgArea44');
	// validateAdditionalInfo('additionalInfo','imageArea55','msgArea55');
// validateEmprAddress('address','imageArea66','msgArea66');
	...
	// validateEmprLocation('location','imageArea99','msgArea99');
		 ...
		 checkEmprVerificationCode();
	   	...
	   	if(joinfreeValidate == 1){
		      ...
		      document.getElementById('nameURL').focus()
		      joinfreeValidate =0;
		      return false;
	     }else if(joinfreeValidate == 2){
		      document.getElementById('emailId').focus()
			  joinfreeValidate =0;
		      return false;
	     }else if(joinfreeValidate == 10){
		      document.getElementById('reEnterEmployeremail').focus()
			  joinfreeValidate =0;
		      return false;
	     }else if(joinfreeValidate == 3){
		     document.getElementById('emplrpassword').focus()
		     ...
		     document.getElementById('confirmEmplrPassword').focus()
		     joinfreeValidate =0;
		     return false;
	     }else if(joinfreeValidate == 7){
		     ...
		     document.getElementById('emprCity').focus()
		     joinfreeValidate =0;
		     return false;
	     }else if(joinfreeValidate == 12){
		     document.getElementById('missionstatementId').focus()
		     joinfreeValidate =0;
		     return false;
	     }
	     ...
	     /*else if(joinfreeValidate == 4){
		     ...
		     return false;
		      ...
		      document.getElementById('state_1.6').focus()
		 	  joinfreeValidate =0;
		      return false;
	     }else if(joinfreeValidate == 9){
		      document.getElementById('phoneNumberId').focus()
	     ...
	     */
		     ...
		     joinfreeValidate = 0;
		     return validateTermsAndConditions('acceptTermsOfServiceWarnMsgEmployer',$('#acceptEmployerCondi').is(':checked'));
	     } 
	}
	
	function validatePhoneNumbernew(txtID,imageArea,messageArea)
	{
	
	     var flag  ="true";    
	     var message=""  
	     var phoneNo=$("#emplrphoneNumberId").val();
	    
	   
	     var phoneRESpace = /^\(\d\d\d\) \d\d\d-\d\d\d\d$/;
	     var phoneRE = /^\(\d\d\d\)\d\d\d-\d\d\d\d$/;
	     var re10digit= /^\d{3}-\d{3}-\d{4}$/; 
	     var nodash10digit= /^\d{10}$/; 
	      ...
	      if(phoneNo=="")
		   ...
		   else if (phoneNo!="" && phoneNo.match(phoneRE))
		   {
	            message=""
	            setMessageForEmprAccountValidation(message,flag,imageArea,messageArea);
	            return flag;
		   }else if(phoneNo!="" && phoneNo.match(re10digit))
		   {
		       
	            message=""
	            setMessageForEmprAccountValidation(message,flag,imageArea,messageArea);
	            return flag;
		   }else if(phoneNo!="" && phoneNo.match(phoneRESpace))
		   {
	            message=""
	            setMessageForEmprAccountValidation(message,flag,imageArea,messageArea);
	            return flag;
   		   }else if(phoneNo!="" && phoneNo.match(nodash10digit))
		   {
	            message=""
	            setMessageForEmprAccountValidation(message,flag,imageArea,messageArea);
	            return flag;
	...
	function validateMissionStateMent(txtID,imageArea,messageArea){
		var minLength = 200; 
		var flag="true";
		var stmt = document.getElementById(txtID).value;
	             
        if (stmt!="" && stmt.length > minLength){
                  ...
                  setMessageForEmprAccountValidation(message,flag,imageArea,messageArea);
                  return false;
              }else if(stmt!="" && trimFieldVal(stmt)==""){// check for only spaces allowed in  
