...
function editBriefcaseDetails(id)
{
    ...
    if(itemName.length>45)
    {
       ...
       $("#success").hide();
       $("#failure").hide();
       $("#validateTitleName").html("<h6>Title should have less than 45 characters.");
...
function deleteBriefcaseURL(){

	document.getElementById("delete").href= "deleteBriefcaseURL.action?item.id="+contactId;
}
...
function pop_upForFlagComment(e,id,name,spanid,pop_up_id,location)
{
...
function storeFlagcommentForThreads() {
     ...
     $.post("storeFlagCommentForThreads.action",{'flagItemId':flagItemId,'flagComment':flagComment,'type':type,'selectedFlagTypeName':selectedFlagTypeName},function (data)
	   {
...
function addBriefcaseURL() 
{
	...
	var checkURL3=nameOfURL.startsWith("http:");
	if(name==""){
		$("#nameId").show();
		$("#nameId").html("<h5>Please enter title.</h5>");
		return false;
	}
	else if(nameOfURL==""){
		$("#nameId").show();
		...
		return false;
	}else if((checkURL==false)&&(checkURL2==false)&&(checkURL3==false)){
		$("#nameId").show();
...
function editBriefcaseDetailsURL(id){
$("#failureMsg").html("");
  ...
  //  var checkURL=itemName.startsWith("http://www.");
    ...
    if(title=="")
    {
       ...
       $("#success").hide();
       $("#failure").hide();
       $("#failureMsg").html("<h6>Please enter title </h6>");
       $("#validateTitleName").show();
      return false;
    }
   $.post("editBriefcaseItemsURL.action",{'item.id' :id,'name':title,'itemURL':itemName,'privacySetting':itemPrivacySetting},function (data)
   {      
