...
<div id="bc_main_outer">
 <s:hidden name="inbox.id" value="%{inbox.id}"/>
	<div id="bc_main_mid">
		...
		<a href="${ctx}/employer/employerInbox.action">Inbox<s:if test="unreadMsgListSize!=0">(<s:property value="unreadMsgListSize"/>)</s:if></a></li><li><a href="${ctx}/employer/employerOutBox.action">Outbox</a></li><li class="align-right"><a href="${ctx}/employer/composeEmployerMsg.action">Compose a New Message</a></li></ul></div></div>
				...
				<div class="bc_lb-content">
				  ...
				  <div class="row2">			
                   ...
                   <br clear="all" />
                    ...
                    <input type="hidden" name="inbox.id" value="<s:property value="inbox.id"/>" id="emprinBoxReplyId">
                    <s:a href=" " id="msgReadInboxEmprid">
                    <img src="${ctx}/styles/default/images/msg_reply.gif" width="131" height="33" onclick="sendEmprReadMsgInboxReply();" /></s:a>
                    </div>
                    
                    <div style = "float: left; text-align:right; width: 500px; font-size: 10px;">
                    <input type="hidden" name="inbox.id" value="<s:property value="inbox.id"/>" id="emprinBoxId">
                    <s:a href=" " id="deleteEmprReadMsg" onclick="deleteEmprRecdReadMsgs()">Delete</s:a>
                    </div>
                    <br /><br />
				</div></div></div></div></div>
			</div>
